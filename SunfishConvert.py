...
'''
This code uses integer representations for squares on a chess board to
calculate movements as well as the common a-h, 1-8 notation.
...
class Piece(object):
    ...
    def __init__(self, label, past_moves, current_pos, movements,
        ...
        self.current_pos = current_pos  # Tuple w/ algebraic notation and int
...
def pawn_promo(new_space, player, label, board):
    '''
    ...
    for piece in board:
        ...
        if (piece.player == player) and (piece.label == "P") \
                and (piece.current_pos[0] == prev_pos):
            if label == "Q":
                board.append(Piece("Q", piece.past_moves, piece.current_pos,
                                   FREE_MOVEMENT, True, piece.player))
            elif label == "N":
                board.append(Piece("N", piece.past_moves, piece.current_pos,
                                   KNIGHT_MOVEMENT, False, piece.player))
            elif label == "R":
                movement = UP + DOWN + SIDE
                board.append(Piece("R", piece.past_moves,
                                   piece.current_pos, movement, True,
                                   piece.player))
            elif label == "B":
                board.append(Piece("B", piece.past_moves,
                                   piece.current_pos, DIAGONAL, True,
                                   piece.player))
            board.remove(piece)
            return sunfish_move
...
def convert_string(move, turn, board):
    ...
    if move[-2] == "=":  # If pawn promotion
        # Pawn promotion notation is a8=Q format
                        ...
                        if (movement % factor == 0)\
                            ...
                            for i in range(1, (movement // factor) + 1):
                                ...
                                for subpiece in board:
                                    ...
                                    if (subpiece != piece) \
                                        and (subpiece.current_pos[1] == test):
                                        blocked = True
                            if not blocked:
