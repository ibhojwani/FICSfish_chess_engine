...
def create_sunfish_board(string, next_player="White"):
    '''
    ...
    Inputs:
        ...
        next_player: String "White" or "Black" that says which player is next

    Returns:
        A sunfish board represented by a string
    '''
    board = SunfishConvert.played_board(
        string)  # Converts string of moves to end board position
    # Turns string (blank Sunfish board) to list for element reassignment
    new_board = list(BLANK_BOARD)
    for piece in board:  # Places pieces on board
        place = piece.current_pos[1]
        index = index_conversion(place)
        if piece.player == next_player:
...
def modified_sunfish(string, next_player="White"):
    '''
    ...
    Returns:
        ...
        EX e2e4
    '''
    sunfish_board = create_sunfish_board(string, next_player)
    searcher = sunfish.Searcher()
    pos = sunfish.Position(sunfish_board, 0, (True, True), (True, True), 0, 0)
    # move returned as a tuple of integers
    ...
    if next_player == "Black":
        coordinate_from = coordinate_from[0] + \
            (FLIPPED_DICT[coordinate_from[1]])
